...
import Database (runPool, withPool, withDB)
...
import Types (CommandRunner, Handler, MkUrl)
...
runner :: PathInfo r
       ...
       -> IO ()
runner route on500 migration pop = do
  args <- getArgs
  ...
  case config of
    Just conf -> run command route on500 migration pop conf
...
runserver route on500 _ _ conf = do
  ...
  withPool (fromString connstr) poolsize $
    runSettings settings . lambdadelta route on500 conf
...
lambdadelta :: PathInfo r
            => (r -> Handler r)      -- ^ Routing function
            -> (String -> Handler r) -- ^ Top-level error handling function
            -> ConfigParser         -- ^ The configuration
            -> ConnectionPool       -- ^ Database connection reference
            -> Application
lambdadelta route on500 conf = let webroot = get' conf "server" "web_root"
